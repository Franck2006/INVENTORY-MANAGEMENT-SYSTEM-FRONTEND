...
@Component({
    selector: 'app-dev-app-date-input',
    standalone: true,
    ...
    providers: [
        {
            ...
            useExisting: forwardRef(() => DevAppDateInput),
            multi: true,
        },
    ],
})
export class DevAppDateInput implements ControlValueAccessor {
    readonly id = input<string>('date-input-' + Math.random().toString(36).substring(2, 9));
    ...
    readonly inputClasses = computed(() => {
        ...
        const base =
            'peer w-full h-12 pt-4 pb-1.5 pl-4 pr-10 rounded-xl font-medium text-sm text-slate-100 bg-[#1C2541] border transition-all duration-200 outline-none focus:ring-2 disabled:pointer-events-none disabled:opacity-40 placeholder-transparent';
